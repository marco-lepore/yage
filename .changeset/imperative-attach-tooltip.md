...
- **`@yagejs/ui` — `attachTooltip(anchor, scene, { content, placement, offset, maxWidth })`**. Imperative, headless tooltip that parents a content node into the scene overlay and anchors it to any UI primitive (`UIPanel._node`, `UIButton`, `UIImage`, …). It builds the floating parts and returns a `{ setActive, dispose }` controller — it wires **no input of its own**, so it never clobbers the anchor's handlers. You drive activation: hover is a one-liner when the anchor has no `onHover` of its own (`anchor.update({ onHover: tip.setActive })` — `update()` replaces that single slot, so compose explicitly if it already has one), or trigger from focus / long-press / programmatically. `anchor` is read only for positioning, and `setActive` stays a safe no-op after `dispose()`. Works in a pure imperative scene with **no `<UIRoot>` / React** — diegetic UI like `ScreenFollow` namecards can now have tooltips.
