...
- **`@yagejs/ui` — `attachTooltip(trigger, scene, { content, placement, offset, maxWidth })`**. Imperative, headless tooltip for any UI primitive (`UIPanel._node`, `UIButton`, `UIImage`, …). Subscribes through the trigger's `watchHover` (the additive hover channel below) so the tooltip composes with — never clobbers — the element's own `onHover`, parents a content node into the scene overlay, and returns a `dispose()` that drops the tooltip's hover subscription (leaving any other hover handlers intact) and releases the slot. Works in a pure imperative scene with **no `<UIRoot>` / React** — diegetic UI like `ScreenFollow` namecards can now have tooltips.
- **`@yagejs/ui` — `watchHover(fn): () => void`** on every interactive primitive (and on the shared `PointerEvents` helper). Hover now has two tiers: the swap-in-place `onHover` prop (declarative / reconciler) and `watchHover`, an **additive** subscription that stacks alongside it and returns an unsubscribe. Imperative hover-driven add-ons (like `attachTooltip`) build on `watchHover` so they no longer overwrite the element's own `onHover`.
