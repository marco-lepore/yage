...
- **Game-state binding model**: `play(script, binding)` is the single bridge between a game and a conversation. A script declares conversation-local `vars` (mutable, written by `set` / `ctx.setVar` / `handle.setVar`) and read-only `external` game state (`name → "string" | "number" | "boolean"`, supplied as a constant or a live getter); conditions and `{token}` interpolation read both from one namespace, resolved at present-time so an earlier command's effect shows on a later line. Commands are handled via a `commands` map + optional `fallbackCommand` (still mirrored to `DialogueCommandEvent`). `defineScript()` is the TS-first authoring path — it infers var/external types so `play()` requires a matching binding and returns a typed `DialogueHandle` (`setVar` / `getVars`). Scripts are validated at load-time (references/types) and play-time (binding coverage), throwing `DialogueScriptError` / `DialogueBindingError`. (Replaces the earlier `params` + `onCommand` controller options.)
