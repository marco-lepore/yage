...
A `UIText` given both a definite `width` and a definite `height` now wraps to that width. Yoga calls no measure function when neither axis is left to measure, and that callback was the only place word wrap was switched on, so a fully sized text rendered one unbroken line.
