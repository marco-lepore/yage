...
`configure(options)` sets the emitter's configuration from now on: `lifetime`, `speed`, `angle`, `scale`, `alpha`, `rotation`, `rotationSpeed`, `tint`, `spawnOffset`, `radialSpeed`, `rate`, `gravity`, `damping` and `blendMode`. Particles already in flight keep the values they were spawned with, and continuous emission picks the new values up on its next particle.
