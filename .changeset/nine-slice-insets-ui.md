...
The insets are read from the background options, not from the texture's own metadata. A background given a new `texture` without a `nineSlice` beside it keeps the insets from the options that last stated them.
