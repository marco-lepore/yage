...
`@yagejs/core` gains a `LocalizationAdapter` seam — wrap your i18n library (i18next, FormatJS, Fluent, or the built-in `Intl`) — plus a plain-data `LocalizedBinding`, a `msg()` factory, and a `LocalizationPlugin` that re-renders bound text when the locale changes. YAGE owns the reactive binding; the library owns resolution and the catalog format.
