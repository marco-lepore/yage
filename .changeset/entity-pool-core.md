...
- A pool owns its members' lifetimes: `entity.destroy()` on a member throws and names `release` instead, as does destroying an entity that has a member below it. Only `dispose()` destroys members. A member destroyed behind the pool's back would otherwise be handed out dead on the next acquisition.
