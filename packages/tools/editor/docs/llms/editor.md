...
export default defineEditorConfig({
  ...
  levels: ["src/levels/*.yage-level.json"], // globs, confined to the Vite root
...
viewport takes the middle, the **inspector** is the right column, and the
**Actors** strip sits under the viewport. The strip starts closed and its
header opens it; open, it takes its height from the viewport rather than
covering it, its entries wrap into rows under their group headings, and past a
few rows it scrolls down — never sideways, which on a trackpad or touchscreen
is the browser's back gesture. Panels are not resizable and none of these positions is
configurable.
...
| Delete button, Delete, or Backspace                                    | Removes the selection and everything authored under it                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `F` / `Shift`-`F`                                                      | Frames the selection / puts the view back at the origin at zoom 1                                                                                                                                                                                                                                                                                                                                                                                                       |
...
**The drawn gizmo holds still while a gesture runs.** A gesture pivots about
the point it took at the press, and the rectangle round a turning arrangement
breathes as it turns, so a pivot recomputed each frame would wander off the one
in use.
...
declared parameter, any finding about the placement, and an **In your game**
section holding **Key** — all of which vary with the type.

The inspector's order is: the placement's type and id, the parameter controls,
the findings, then **In your game**.
...
The only parameter kind that ships is `param.asset()`, rendered as a text field
holding the project-relative path, with the project's own files on offer under
it. **▾** opens the list, typing narrows it by case-insensitive substring, and
...
**Typing a value.** `param.asset()` is the one parameter kind with a published
control. The active flag, the parent, and extensions have no controls at all.
