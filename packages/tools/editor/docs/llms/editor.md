...
| Inspector pair field                                                   | Two number fields, `x` and `y`, for a `vec2` or a `point`; typing into either commits the whole pair. While the point's ring is dragged in the viewport, both follow it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| Point handle in the viewport                                           | Selecting one placement draws a ring at each of its `point` parameters, in a colour of its own, with a dashed line back to the placement's origin for a `relative` one. Hovering it names the parameter. Dragging it moves the value; the release is one edit and one undo step. `Snap` lands it on the grid, `Alt` holds it off, and `Shift` keeps the drag to one axis of the value's own frame — the placement's for a relative point, the world's otherwise. A press on the ring wins over the gizmo under it                                                                                                                                              |
...
and typing into either commits the whole pair. A point is also a place, so
selecting the placement draws a ring where the value is and dragging the ring
moves it; the two fields follow, and the release is one edit. A point declared
`relative: true` is measured from the placement's own origin and drawn with a
dashed line back to it, so it travels with the placement; one without the line
is a world point that stays where it is. The ring lands on the grid under
