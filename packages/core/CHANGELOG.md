...
- [#391](https://github.com/marco-lepore/yage/pull/391) [`0c90d77`](https://github.com/marco-lepore/yage/commit/0c90d774bdbda47f5a95c92ab7aef11d7a19e7b9) Thanks [@marco-lepore](https://github.com/marco-lepore)! - Correct three documentation examples: the `SceneManager.preload` progress bar uses `UIProgressBar.update({ value })`, the `Inspector.drive` example handles a missing entity position, and `Entity.requireKey` names the entity's `setup()` or a component's `onAdd()` as the place to call it.

- [#388](https://github.com/marco-lepore/yage/pull/388) [`6888d06`](https://github.com/marco-lepore/yage/commit/6888d06c6fdf2361f41c5521ebdda83dc833b6c4) Thanks [@marco-lepore](https://github.com/marco-lepore)! - The README sample and the JSDoc show the recommended patterns: the README marks `Blueprint` deprecated and puts the sample's logic in a component, the `EntityPool` example creates its pool in a component's `onAdd()`, and `Scene.registerScoped` no longer suggests it for game state. `ProcessSlotConfig.loop` notes that a looping slot never completes, so its `onComplete` never runs; loop a `Sequence` for a repeating callback.
...
- [#376](https://github.com/marco-lepore/yage/pull/376) [`908622a`](https://github.com/marco-lepore/yage/commit/908622adcf1a401251539e9edd081ad7ffc7e642) Thanks [@marco-lepore](https://github.com/marco-lepore)! - `UINodeSnapshot.state` carries the element's own interaction state. An element that reports one fills the field — a button's focused, hovered, pressed and disabled flags, a checkbox's checked flag — and the field stays `null` for an element that reports none.
...
- [#385](https://github.com/marco-lepore/yage/pull/385) [`5efe5f6`](https://github.com/marco-lepore/yage/commit/5efe5f6de138b71048e6f4752ed74647a9fc3e76) Thanks [@marco-lepore](https://github.com/marco-lepore)! - A `this.sibling(Cls)` reference passes `instanceof Cls`, so an engine API that
...
- [#340](https://github.com/marco-lepore/yage/pull/340) [`56570ae`](https://github.com/marco-lepore/yage/commit/56570ae539b98d2eefa000898c71eabea28df571) Thanks [@marco-lepore](https://github.com/marco-lepore)! - The Inspector's per-entity reads take an entity id as well as a name, so the ids `getEntities()`, `snapshot()` and the event log hand out can be fed straight back in and several same-named entities are individually readable.
...
- [#346](https://github.com/marco-lepore/yage/pull/346) [`1b12043`](https://github.com/marco-lepore/yage/commit/1b120433e9570b21f5748c8cfaaf98bc781c4a62) Thanks [@marco-lepore](https://github.com/marco-lepore)! - Publish the full standard easing table. Alongside `easeLinear`, `@yagejs/core`
...
- [#340](https://github.com/marco-lepore/yage/pull/340) [`56570ae`](https://github.com/marco-lepore/yage/commit/56570ae539b98d2eefa000898c71eabea28df571) Thanks [@marco-lepore](https://github.com/marco-lepore)! - `Scene.use(key)` and `Scene.service(key)` are public, so a scene-scoped service resolves from anything holding a scene reference — an entity's `setup()`, a helper function that takes a `scene`, an addon — and not only from inside the scene subclass.
