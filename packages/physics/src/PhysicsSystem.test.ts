  ...
  class MockWorld {
    ...
    createRigidBody(desc: MockRigidBodyDesc): MockRigidBody {
      const body = new MockRigidBody(
        (desc as unknown as { _type?: string })._type ?? "dynamic",
      );
  ...
  describe("update", () => {
    ...
    it("drives kinematic bodies from the captured target, not the Transform", async () => {
      ...
      const transform = entity.add(
        new Transform({ position: new Vec2(150, 250) }),
      );
    ...
    it("steps two scenes with different timeScales independently", async () => {
      ...
      const mockWorld2 = (
        world2 as unknown as { world: InstanceType<typeof mocks.MockWorld> }
      ).world;
    ...
    it("per-scene accumulators are independent", async () => {
      ...
      const mockWorld2 = (
        ctx2.world as unknown as { world: InstanceType<typeof mocks.MockWorld> }
      ).world;
