...
function candidate(
  overrides: Partial<InteractCandidate> = {},
): InteractCandidate {
  return {
    position: { x: 0, y: 0 },
    radius: 0,
...
describe("selectInteractionFocus", () => {
  it("returns the nearest in-range candidate", () => {
    ...
    const winner = selectInteractionFocus(
      { position: { x: 0, y: 0 }, range: 50 },
      [far, near],
    );
  ...
  it("returns null for empty candidates", () => {
    expect(
      selectInteractionFocus({ position: { x: 0, y: 0 }, range: 10 }, []),
    ).toBeNull();
  });
  ...
  it("a higher priority candidate wins over a nearer lower-priority one", () => {
    ...
    const farButPrioritized = candidate({
      position: { x: 20, y: 0 },
      priority: 10,
      order: 1,
    });
...
describe("rankInteractables", () => {
  it("returns every in-range candidate ordered priority, then distance, then order", () => {
    ...
    const nearLow = candidate({
      position: { x: 5, y: 0 },
      priority: 0,
      order: 0,
    });
    const farLow = candidate({
      position: { x: 20, y: 0 },
      priority: 0,
      order: 1,
    });
    ...
    expect(rankInteractables(query, [nearLow, farLow, high])).toEqual([
      high,
      nearLow,
      farLow,
    ]);
  ...
  it("its first element is exactly what selectInteractionFocus picks", () => {
    ...
    expect(rankInteractables(query, candidates)[0]).toBe(
      selectInteractionFocus(query, candidates),
    );
  });
});
