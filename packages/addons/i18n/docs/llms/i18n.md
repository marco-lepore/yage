...
keys may nest (`{ hud: { hp } }` is `"hud.hp"`). A regional tag (`pt-BR`) uses
...
localization.subscribe(() => …); // after each change; returns the unsubscribe
```
...
component on a dormant entity is skipped and catches up when the entity is
enabled. `plugin.relocalizeAll()` runs the pass on demand.
...
`locale` is `undefined`. `<Trans>` takes every `<Text>` prop plus `message` and
...
```tsx
<Trans message={msg(`quest.${id}.title`, QUESTS.get(id).title)} />
```
