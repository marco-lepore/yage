...
const fills = (ops: readonly DrawOp[]): { color: number; alpha?: number }[] =>
  ops
    .filter((o) => o.op === "fill")
    .map((o) => o.args[0] as { color: number; alpha?: number });

const rows = (labels: string[]): MenuSkinRow[] =>
  labels.map((label, i) => ({
    label,
    rect: { x: 100, y: 40 + i * 21, width: 56, height: 21 },
  }));
...
describe("menuSkin (default action-menu renderer)", () => {
  it("draws a frame and one row entity per action", () => {
    ...
    menuSkin(defaultInventoryTheme()).renderMenu(
      scene,
      MENU_RECT,
      rows(["Use", "Drop"]),
    );
    ...
    const frameOps =
      named(scene, "inv-menu-frame")[0]?.get(GraphicsComponent).lastOps() ?? [];
    expect(fills(frameOps).map((f) => f.color)).toEqual([0x1a1a2e]); // frameColor
  });

  it("draws the highlight bar at the selected row's rect", () => {
    const { scene } = createMockScene();
    const handle = menuSkin(defaultInventoryTheme()).renderMenu(
      scene,
      MENU_RECT,
      rows(["Use", "Drop", "Examine"]),
    );
    handle.highlight(2);

    const barOps =
      named(scene, "inv-menu-bar")[0]?.get(GraphicsComponent).lastOps() ?? [];
...
describe("hints (default scroll-hint renderer)", () => {
  ...
  it("draws both triangles when scrolled off top and bottom", () => {
    const { scene } = createMockScene();
    hints(defaultInventoryTheme()).render(scene, {
      up: true,
      down: true,
      window: WINDOW,
    });
    const ops =
      named(scene, "inv-slots-hints")[0]?.get(GraphicsComponent).lastOps() ??
      [];
  ...
  it("draws one triangle for a single direction and none when in view", () => {
    ...
    const handle = preset.render(scene, {
      up: true,
      down: false,
      window: WINDOW,
    });
  ...
  it("toggles visibility and disposes its entity", () => {
    const { scene } = createMockScene();
    const handle = hints(defaultInventoryTheme()).render(scene, {
      up: true,
      down: true,
      window: WINDOW,
    });
