...
const fillColors = (ops: readonly DrawOp[]): number[] =>
  ops
    .filter((o) => o.op === "fill")
    .map((o) => (o.args[0] as { color: number }).color);
...
describe("iconCell", () => {
  it("draws a background box for an empty slot, no content", () => {
    ...
    const ops =
      named(scene, "inv-cell-bg")[0]?.get(GraphicsComponent).lastOps() ?? [];
  ...
  it("draws a letter tile at the default color when the item has no icon", () => {
    const { scene } = createMockScene();
    iconCell(defaultInventoryTheme()).renderCell(
      scene,
      occupied({ name: "Potion" }),
      RECT,
      false,
    );
  ...
  it("honors a themed tileLetterColor", () => {
    ...
    expect(
      named(scene, "inv-cell-letter")[0]?.get(TextComponent).options.style.fill,
    ).toBe(0x445566);
  });
  ...
  it("uses a sprite when the icon resolves", () => {
    ...
    expect(named(scene, "inv-cell-letter")).toHaveLength(0);
    expect(
      named(scene, "inv-cell-icon")[0]?.get(SpriteComponent).options.texture,
    ).toBeDefined();
  });
  ...
  it("draws a quantity badge only when the stack is > 1", () => {
    ...
    expect(named(b, "inv-cell-qty")[0]?.get(TextComponent).options.text).toBe(
      "7",
    );
  });
  ...
  it("setSelected redraws the background without spawning entities", () => {
    const { scene } = createMockScene();
    const handle = iconCell(defaultInventoryTheme()).renderCell(
      scene,
      occupied(),
      RECT,
      false,
  ...
  it("dispose destroys every entity it spawned", () => {
    ...
    expect(
      [...scene.getEntities()].filter((e) => !e.isDestroyed),
    ).not.toHaveLength(0);
    handle.dispose();
    expect([...scene.getEntities()].filter((e) => !e.isDestroyed)).toHaveLength(
      0,
    );
  });
});
...
describe("rowCell", () => {
  it("draws a name and a right-aligned ×qty", () => {
    const { scene } = createMockScene();
    rowCell(defaultInventoryTheme()).renderCell(
      scene,
      occupied({ name: "Elixir", quantity: 4 }),
      RECT,
      false,
    );
    expect(
      named(scene, "inv-row-name")[0]?.get(TextComponent).options.text,
    ).toBe("Elixir");
    expect(
      named(scene, "inv-row-qty")[0]?.get(TextComponent).options.text,
    ).toBe("×4");
  });

  it("an empty row draws only the bar (blank, still selectable)", () => {
    const { scene } = createMockScene();
    const handle = rowCell(defaultInventoryTheme()).renderCell(
      scene,
      empty,
      RECT,
      false,
  ...
  it("spawns the bar before the text (paints underneath)", () => {
    ...
    expect(order.indexOf("inv-row-bar")).toBeLessThan(
      order.indexOf("inv-row-name"),
    );
  });
