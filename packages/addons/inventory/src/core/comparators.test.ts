...
function entry(
  id: "potion" | "sword" | "apple" | "rock",
  quantity = 1,
): SortEntry {
  return {
    stack: { itemId: id, quantity },
    def: catalog.get(id),
    order: catalog.orderOf(id),
  };
}

describe("comparators", () => {
  it("byCatalogOrder follows authoring order", () => {
    const sorted = [entry("rock"), entry("apple"), entry("potion")].sort(
      byCatalogOrder,
    );
  ...
  it("byName sorts by display name", () => {
    const sorted = [entry("sword"), entry("potion"), entry("apple")].sort(
      byName,
    );
  ...
  it("byCategory groups categories, undefined last, catalog order within", () => {
    ...
    expect(sorted.map((e) => e.def.id)).toEqual([
      "potion",
      "apple",
      "sword",
      "rock",
