...
> extends Component<Parent> {
  ...
  updateTransform() {
    if (this.isEnabled()) {
      ...
      const { prevSimulationTime, simulationTime, gameTimeMS } =
        this.parent.scene.rapier;

      const fullTimestep = simulationTime - prevSimulationTime;
      const factor = (gameTimeMS - prevSimulationTime) / fullTimestep;

      if (body) {
        const { x, y } = body.translation();
        const { x: prevX, y: prevY } = this.prevTranslation;
  ...
  static colliderComponentMap = new Map<
    number,
    RapierBodyComponent<any, any>
  >();
}
