...
<div style="display: flex; flex-direction: column; align-items: center; gap: 0.5rem; margin-bottom: 2rem;">
  ...
  <p style="color: var(--sl-color-gray-3); font-size: 0.85rem; margin: 0;">Self-playing Pong — running live with Rapier 2D physics.</p>
</div>
...
await createGame({
  ...
  scene: defineInlineScene("pong", (scene, { camera }) => {
    ...
    const ball = scene.spawn(Ball, { w: 600, h: 400 });
    scene.spawn(Paddle, { x: 30, y: 200, ball, side: "left" });
    scene.spawn(Paddle, { x: 570, y: 200, ball, side: "right" });
    scene.spawn(Wall, { x: 300, y: -10, w: 600, h: 20 });      // top
    scene.spawn(Wall, { x: 300, y: 410, w: 600, h: 20 });       // bottom
    scene.spawn(Goal, { x: -20, y: 200, w: 20, h: 400, side: "left" });
    scene.spawn(Goal, { x: 620, y: 200, w: 20, h: 400, side: "right" });
  }),
});
```
  </TabItem>
...
```typescript
import { Entity, Component, Transform, Vec2, ProcessComponent, ProcessSlot } from "@yage/core";
import { GraphicsComponent } from "@yage/renderer";
import { RigidBodyComponent, ColliderComponent } from "@yage/physics";
