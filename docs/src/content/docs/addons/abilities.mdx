...
Open hitbox, guard, invulnerability, slow-motion, and stagger windows release
their live effects while dormant. Enabling the component again resumes the
same activation and clocks.
...
If a custom window owns a live resource, add `suspend` and `resume` hooks.
`suspend` temporarily releases the resource when `Abilities` becomes dormant;
`resume` restores it without reopening the window or resetting its clock.
