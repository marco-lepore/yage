...
function installChargeAbility(abilities: Abilities) {
  ...
  return {
    begin: () => abilities.send("charge"),
    release: () => abilities.release("charge"),
  };
}
```

Call `installChargeAbility` once. Call the returned `begin()` when charging
starts and `release()` when the player releases the control.
...
function frameAbilities(entity: Entity, defs: readonly AbilityDef[]) {
...
function bindAbilities(entity: Entity) {
  entity.add(
...
on exit. Call the returned `dispose` function if the pool should end earlier.
...
function elementalAttack(
  ...
  const burn = hitbox<ElementHit>({
    ...args,
    hit: { element: "fire", damage: 8 },
  });
  return { receiver, burn };
}
```
...
function elementalFeedback(
  attacker: Entity,
  args: Omit<HitboxStepArgs<ElementHit>, "hit">,
  resist: HitStage<ElementHit, HitReceiver<ElementHit>>,
  applyElementFeedback: (element: ElementHit["element"]) => void,
) {
...
```ts
import type { Entity, Component } from "@yagejs/core";
...
```ts
import type { Entity } from "@yagejs/core";
import { Health, HitReceiver, type Hit } from "@yagejs-addons/abilities";
function ignoreDead(entity: Entity) {
  ...
  entity.add(
    new HitReceiver({
      team: "enemy",
      filter: (hit, receiver) =>
        !health.isDead && sameTeamAllowed(hit, receiver),
    }),
  );
}
```
