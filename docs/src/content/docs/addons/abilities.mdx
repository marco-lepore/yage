...
`KeyframeAnimator` whose playback runs on rendered-frame time, and any tween
a step starts keeps the frame clock unless scheduled otherwise. Input is
sampled once per rendered frame, so an intent sent this frame starts a
timeline that then advances on the fixed step.
