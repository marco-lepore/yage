...
const WolfDiedEvent = defineEvent<void>("game:wolf-died");
...
```ts yage-group="herbs"
function connectQuestHud(
  hud: { toast(message: string): void },
  tracker: Map<string, string>,
) {
  log.on("questCompleted", ({ questId }) => {
    hud.toast(`Quest complete: ${quests.tryGet(questId)?.title ?? questId}`);
...
function mirrorQuests(
  player: Entity,
  scene: Scene,
  achievements: { check(questId: string): void },
) {
...
const save = createSave({
  adapter: localStorageAdapter({ namespace: "quests-demo" }),
});
