...
punches, visibility, cue composition, slow motion, animation, callbacks,
shockwaves, camera modifiers, and a custom effect. Press `N` or `P` to move
between scenes with a slide transition.
...
If that code writes to serializable game state, YAGE saves the resulting value.
For custom visual movement, write to a renderer modifier handle and release the
handle in `finish`.
</Aside>
