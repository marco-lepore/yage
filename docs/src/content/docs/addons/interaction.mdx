...
`focus` is simply `inRange[0]` — one snapshot, not two sources of truth.
...
distances, so a negative total would come back positive and start matching
distant targets. Rather than pay for that check on every candidate every frame,
`Interactor` and `Interactable` each warn once (in dev) when they're added with
a negative value. Use `range: 0` to reach only what the interactor overlaps.
</Aside>

## Multiple targets, selection UI, and highlighting

`focus` is the single default target. Behind it, `interactor.inRange` is the
full set of in-range, enabled interactables **in the same order focus is picked
by** — so `inRange[0]` *is* the current `focus`. Both come from one snapshot
rebuilt each `update()`, so they can never disagree; it reads empty before the
first frame and while the interactor is disabled.
...
and no facing-cone. A physics-sensor discovery source, facing-cone priority, and
a bundled world-anchored prompt presenter are deferred; the focus event and
`inRange` already carry what a presenter would draw, so one stays purely
additive later.
</Aside>
