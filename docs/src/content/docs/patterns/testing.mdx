...
| `getErrors()` | Systems/components disabled by the error boundary, plus recorded callback failures (`callbackErrors`) — populated only under `errors: "isolate"`; empty under the default `"fatal"`, which stops the engine and rethrows instead |
