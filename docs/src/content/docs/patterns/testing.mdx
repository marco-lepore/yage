...
snapshot. Each graphical component carries a derived `render` block with
world-space `bounds` and component-local `visible`, computed on demand from the
live display object. `SplitTextComponent` also reports per-glyph visibility and
the visible substring, so a typewriter reveal is verifiable from the public API
