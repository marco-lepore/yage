...
world-space `bounds` and resolved `visible`, computed on demand from the live
display object. `SplitTextComponent` also reports per-glyph visibility and the
visible substring, so a typewriter reveal is verifiable from the public API
