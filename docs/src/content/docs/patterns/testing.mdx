...
describe("GravitySystem", () => {
  it("should apply gravity to entities with velocity", () => {
    ...
    // Run one fixed update at 0.016s
...
specified number of times with a fixed ~16ms delta (which the loop converts
to ~0.0167s for `update(dt)`). No `setTimeout` or `requestAnimationFrame`
involved.
...
describe("Process timing", () => {
  it("should fire callback after delay", () => {
    ...
    let fired = false;
    pc.run(Process.delay(0.1, () => { fired = true; }));

    // Advance 0.05s — not enough
    pc._update(0.05);
    expect(fired).toBe(false);

    // Advance another 0.06s — total 0.11s, past the 0.1s delay
    pc._update(0.06);
...
it("should tween alpha to zero", () => {
  ...
  pc.run(Tween.to(obj, "alpha", 0, 0.2, easeLinear));

  pc._update(0.1); // halfway
  expect(obj.alpha).toBeCloseTo(0.5);

  pc._update(0.1); // complete
...
The Inspector is a runtime introspection and control API that auto-attaches to
...
| `getErrors()` | Systems/components disabled by the error boundary (`errors: "isolate"` only), plus recorded failures (`callbackErrors`) — populated under both policies; under the default `"fatal"` it holds the one failure that stopped the engine, with `outcome: "fatal"` |
