...
A scene class **assembles** the scene: it preloads assets, sets up the camera
and spawns entities. It holds no game state and no rules. Geometry belongs in
entity classes. Movement, collision callbacks, event handlers and game state
such as the score belong in those entities' components.

Rule of thumb: if a scene grows past ~150 lines, that's a signal you're
inlining entity logic. Extract entity classes and components until `onEnter`
is a list of spawns.

```typescript
import { Entity, Scene, defineEvent } from "@yagejs/core";
...
class GameScene extends Scene {
  ...
  onEnter() {
    this.spawn(Hud, { key: HUD_KEY }); // hosts the score
    this.spawn(Player, { x: 120, y: 400 }); // respawns itself on PlayerHit
...
// Asset handles and the event, declared with the scene:
const playerTexture = texture("/assets/player.png");
const jumpSfx = sound("/assets/jump.wav");
const PlayerHit = defineEvent("game:player-hit");

// Defined in entities/ and elsewhere in the game:
declare class Hud extends Entity {}
declare const HUD_KEY: string;
declare class Player extends Entity { setup(params: { x: number; y: number }): void }
declare class Platform extends Entity { setup(params: { x: number; y: number; width: number; height: number }): void }
declare class Coin extends Entity { setup(params: { x: number; y: number }): void }
declare class Hazard extends Entity { setup(params: { x: number; y: number; amplitude: number; period: number }): void }
```

The player's controller component reacts to `PlayerHit` itself, with
`this.listenScene(PlayerHit, () => this.respawn())`. When a component should
not listen for itself, `onEnter` may connect the event to a component method in
one line, such as `this.on(PlayerHit, () => player.get(Respawn).respawn())`.
[State Management](/patterns/state-management/) shows where the score lives.
...
export class Oscillate extends Component {
  constructor(readonly options: { axis: "x" | "y"; amplitude: number; period: number }) {
    super();
  }
...
```typescript yage-group="layout" yage-file="main.ts"
// main.ts — entire file
...
```typescript yage-group="layout" yage-file="scenes/GameScene.ts"
// scenes/GameScene.ts
