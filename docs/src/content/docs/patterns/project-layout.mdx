...
The scene excerpt below uses these game-specific contracts. Entity implementations
are omitted because this page shows module layout; the starter contains their
rendering, movement, and collision behavior.

```typescript yage-group="layout" yage-file="entities/contracts.ts"
import { Entity, type Scene } from "@yagejs/core";

export declare class Player extends Entity {
  setup(params: { x: number; y: number }): void;
}
export declare class Platform extends Entity {
  setup(params: { x: number; y: number; width: number; height: number }): void;
}
export declare class Coin extends Entity {
  setup(params: { x: number; y: number }): void;
}
export declare class Hazard extends Entity {
  setup(params: {
    x: number;
    y: number;
    amplitude: number;
...
export declare function respawnPlayer(scene: Scene): void;
```
...
export class Oscillate extends Component {
  ...
  private elapsed = 0;
  constructor(
    private options: { axis: "x" | "y"; amplitude: number; period: number },
  ) {
    super();
  }
  onAdd() {
  ...
  update(dt: number) {
    this.elapsed += dt;
    const offset =
      Math.sin((this.elapsed * Math.PI * 2) / this.options.period) *
      this.options.amplitude;
    this.transform.setPosition(
      this.origin.x + (this.options.axis === "x" ? offset : 0),
      this.origin.y + (this.options.axis === "y" ? offset : 0),
    );
  }
}

// In each entity's setup(), after adding Transform:
function addCoinMotion(coin: Entity) {
  coin.add(new Oscillate({ axis: "y", amplitude: 4, period: 1.2 }));
}
function addHazardMotion(hazard: Entity) {
  hazard.add(new Oscillate({ axis: "x", amplitude: 60, period: 2.2 }));
}
```
