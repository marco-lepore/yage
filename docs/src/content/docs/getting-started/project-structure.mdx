...
| Package | Role |
|---|---|
| `@yagejs/core` | ECS, math (`Vec2`, `Transform`, `MathUtils`), events, scheduling, component base class |
| `@yagejs/renderer` | PixiJS v8 rendering pipeline, sprites, graphics, cameras |
| `@yagejs/lighting` | Radial lights, light-level queries, and lighting renderers |
...
| `@yagejs/pathfinding` | Grid A\* pathfinding, with an adapter for tilemaps |
| `@yagejs/level` | Level documents: read, validate, and load into a scene |
