...
- **`ySortBy` with a shared entity key** — instead of each part keying
  off its own world-y, point `offsetOf` at the *owner entity's*
  footprint so all of an entity's parts collapse to the same key and
  sort as a single unit against the world:

  ```typescript
  // Each part's container carries the OWNER entity's footprint y.
  const sort = ySortBy(
    (c) => (c as { entityFootY?: number }).entityFootY,
  );
