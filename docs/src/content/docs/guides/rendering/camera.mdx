...
`centerOn: { width, height }` does the half-viewport math for you and is
the cleaner way to spell the same thing:

```typescript
this.spawn(CameraEntity, { centerOn: { width: 800, height: 600 } });
```

### Static cameras (`fit: "static"`)

By default a `CameraEntity` is `fit: "follow"` — it tracks a target each
frame, smooths into deadzones, runs bounds, etc. For arcade-style
single-screen games, puzzle grids, or any layout where the camera is
placed once and never moves, set `fit: "static"`:
...
Static cameras silently drop `follow` / `smoothing` / `offset` / `deadzone`
on the floor, so a half-finished refactor that still passes those values
doesn't accidentally re-enable tracking. Shake, zoom, and bounds still
work — `fit` only controls the per-frame *position* follow loop.
...
`LayerDef.sort` to a comparator and `DisplaySystem` re-sorts the layer's
children with it every frame, after transform sync, before camera
transforms:
...
`ySort` is `(a, b) => a.position.y - b.position.y` — terse enough that
you can write your own comparator if you'd rather drive depth from a
different axis or off a custom field on the sprite.

#### Sort offsets (`ySortBy`) — Godot's `y_sort_origin` pattern
...
that gets added to `position.y` before comparison. This is the same
idea as Godot's `y_sort_origin`:
...
sprite.sprite.depthOffset = 32; // sort key is `position.y + 32`
```
