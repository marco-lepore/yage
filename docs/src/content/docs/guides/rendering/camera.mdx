...
#### Multi-sprite entities can split (no per-entity sort group)

`sort` operates on **one key per direct child of the layer**. Every
sprite, text, and graphics is a direct child — positioned at its
entity's world coordinates — so an entity built from *several* sprites
(or a parent entity with child entities, each its own container)
contributes several independent keys, not one. There is no per-entity
stacking context, so a foreign entity whose key lands between two of
your entity's parts will render **between** them:
...
- **Same key + insertion order** — give every part of the composite the
  *same* sort key (build it at one position and offset parts visually
  via each sprite's local position rather than its container's world-y).
  Parts that share a key keep their relative order through the
  stable-sort insertion-order tie-break, so the composite stays
  contiguous and can't be split. The limit: because all parts share one
  key, the group can't itself span a depth range against the world — use
  explicit `zIndex` on parts if you need a guaranteed within-group order.
- **Shared entity key via a custom `sort`** — `ySort`/`ySortBy` key off
  each container's *own* `position.y` (`ySortBy` only *adds* a per-part
  offset, so parts at different world-y still get different keys). To
  collapse a whole entity to one key, write a `sort` fn that reads an
  absolute per-entity depth off each container and only falls back to
  `position.y` when none is set. Tag every part of an entity with the
  *same* owner key and they sort as a single unit against the world:

  ```typescript
  import type { LayerSortFn } from "@yagejs/renderer";

  // Each part's container carries the OWNER entity's footprint y as an
  // absolute sort key — not an additive offset.
  ...
  This trades away per-part interleaving (e.g. a character walking
  behind only a tree's lower trunk) for atomic grouping — pick the
  behavior per layer.

A first-class per-entity `SortGroupComponent` — a sub-container whose
children sort internally while the group participates in the layer
`sort` as one unit, like Unity's `SortingGroup` or Godot's nested
y-sort scopes — is tracked as a follow-up in
[issue #104](https://github.com/marco-lepore/yage/issues/104).
