...
### Filter isolation with `isRenderGroup`
...
uniform scope, which matters whenever you mix component-scope filters
with sibling layers whose pipes read shader uniforms directly.

The classic failure mode comes from `@pixi/tilemap`'s `TilemapPipe`,
which composes `uProjection × uWorldTransformMatrix × tilemap.worldTransform`
on each draw. If a filter elsewhere in the tree has just pushed its own
`uWorldTransformMatrix` onto the global uniform stack, the tilemap pulls
that polluted matrix back and your canopy drifts as soon as a goblin's
hit-flash filter fires.

Flip `isRenderGroup: true` on (a) layers that host filtered content AND
(b) any sibling layer whose pipe must stay unaffected — typically a
tilemap canopy or anything else built on a custom render pipe:
...
readonly layers: readonly LayerDef[] = [
  ...
  // Canopy reads from a tilemap pipe — isolate it so the filter above
  // doesn't leak `uWorldTransformMatrix` into its draw.
  { name: "canopy",  order: 10,  isRenderGroup: true },
  { name: "ui",      order: 100, space: "screen" },
];
```

Render groups have a small fixed cost (their own render pass and
instruction set), so flip the flag on only where you need it. Default:
`false`.
