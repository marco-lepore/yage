...
Seventeen presets ship in `@yagejs/effects`, all save-aware and all using
...
`bulgePinch.setStrength`, `halftone.setAmount`, `wave.setAmplitude`, etc.)
preserve the current intensity ratio, so adjusting the ceiling mid-pulse
...
filter and expose typed extras specific to it. Two — `halftone` and `wave`
— ship as custom WebGL + WGSL shader pairs with no `pixi-filters` dep,
which makes them the smallest reference implementations to copy when
authoring an effect from scratch.
