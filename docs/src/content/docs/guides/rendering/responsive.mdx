...
user-gesture handler (click, touch, key) — browsers reject silently
otherwise. The `webkitRequestFullscreen` variant on older iOS returns
`void` rather than a `Promise`; the wrapper handles both.
