...
```typescript yage-context="context,entity"
import { RendererKey, TextComponent } from "@yagejs/renderer";

const renderer = context.resolve(RendererKey);
const scoreLabel = entity.get(TextComponent).text;
...
const bgGraphics = this.spawn("background").add(
  new GraphicsComponent(),
).graphics;
const fogGraphics = this.spawn("fog").add(new GraphicsComponent()).graphics;
const hud = this.use(SceneRenderTreeKey).get("ui").container;
