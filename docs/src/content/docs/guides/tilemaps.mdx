...
| Tileset `objectalignment` override                 | Not read — tile objects stay bottom-left, and `validateTiledMap()` warns |
...
map itself parses fine: runtime tile editing, a tile object's undrawn image,
per-tile collision shapes and `tilerendersize`.
...
through, so match on the codes your game cannot live with:
`tileset-object-alignment` is a warning, and it means the map's tile objects
sit somewhere other than where Tiled draws them.
...
Its position is measured differently from every other object's. Tiled anchors
a tile object at its **bottom-left** corner, matching how tiles land in tile
layers, so `obj.y` is the object's bottom edge, while every other object type
measures `y` from its top. For an unrotated object, subtract the height to get
the corner a sprite starts from:
...
for (const obj of tilemap.getObjects("props")["Tree"] ?? []) {
  ...
  this.spawn(TreeEntity, {
    tileId: id,
    x: obj.x,
    y: obj.y - obj.height,               // top-left of the drawn tile
    flippedHorizontally,
  });
}
```

Tiled turns a tile object about that same bottom-left corner, so once
`obj.rotation` is non-zero the drawn box swings away from `obj.y - obj.height`
and the subtraction no longer finds it. Put the sprite on the corner itself
instead: anchor it bottom-left, and give the entity's `Transform` the object's
own position and rotation. Position and rotation belong on the Transform —
the renderer copies them onto the sprite every frame, so a `rotation` set on
the sprite is overwritten.
...
That lands where Tiled shows it at any angle.
[Collision extraction](#collision-extraction) does the equivalent internally,
so its rect and the sprite agree.

The `gid` keeps Tiled's flip bits, exactly like the GIDs in raw layer data, so
`readTileGid` splits it the same way.

Two things are not read. Collision shapes authored on the tile inside the
tileset are ignored, so collision extraction treats a tile object as its full
box. A tileset's `objectalignment` override is ignored too — tile objects stay
bottom-left, which is what Tiled does for orthogonal maps unless you change it
— and `validateTiledMap()` reports a `tileset-object-alignment` warning for a
tileset that sets it.
...
| Tile object (carries a `gid`) | `{ type: "rect", x, y, width, height, rotation? }` covering the tile's drawn box, measured up from the bottom-left corner Tiled anchors it at |
