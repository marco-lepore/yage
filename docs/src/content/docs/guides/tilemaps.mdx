...
| Collision-shape extraction → Rapier colliders      | Supported   |
...
through the debug overlay. The full Tiled-collision-to-Rapier wiring
(extracted shapes → static `RigidBodyComponent` + `ColliderComponent`)
is shown inline in the [Collision Extraction](#collision-extraction)
section below; for polygon shapes, pass them through the
`toPhysicsColliders()` helper to convert top-left-origin Tiled rects
into center-origin Rapier configs.
