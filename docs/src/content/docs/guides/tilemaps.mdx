...
Nothing in the "Not yet" rows fails silently. `validateTiledMap()` reports
each one against a map's raw JSON, and `TilemapComponent` logs whatever it
finds when the map loads — see [Checking a Map](#checking-a-map).
...
instead — anchor `{ x: 0, y: 1 }`, position `obj.x, obj.y`, rotation
`obj.rotation` converted to radians — and it lands where Tiled shows it at any
angle. [Collision extraction](#collision-extraction) does the equivalent
internally, so its rect and the sprite agree.
