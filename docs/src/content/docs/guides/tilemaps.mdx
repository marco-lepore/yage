...
An animation outside that shape renders its first frame and reports an
`unsupported-tile-animation` warning saying which tile and why: durations that
differ, frames scattered around the image, or a collection-of-images tileset.
