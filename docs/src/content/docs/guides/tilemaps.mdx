...
| Tiles larger than the map grid                     | Supported — anchored bottom-left, as Tiled draws them |
| Tile objects (an object that draws a tile)         | Supported   |
| Tileset `objectalignment` override                 | Not read — tile objects are always bottom-left |
...
### Tiles bigger than the grid

A tileset can hold tiles larger than the map's grid — a three-cell-tall pillar
on a 16px map, say. Tiled anchors such a tile to the bottom-left of the cell it
was painted in, so a taller tile overhangs upward and a wider one to the right,
and the tile still belongs to that one cell. Rendering follows the editor, so
what you paint is what you see.
...
object** — an object that carries a picture. It arrives with a `gid`, the
global ID of the tile it shows, and everything else a normal object has:
a class, a name, custom properties, a size you can stretch.

The catch is where it sits. Tiled anchors a tile object at its **bottom-left**
corner, matching how tiles land in tile layers, so `obj.y` is the object's
bottom edge. Every other object type measures `y` from its top. Subtract the
height to get the corner a sprite starts from:
...
Two things a tile object does *not* bring with it. Collision shapes authored on
the tile inside the tileset are not read, so
[collision extraction](#collision-extraction) treats a tile object as its full
box. And a tileset's `objectalignment` override is not read either — tile
objects are always placed bottom-left, which is what Tiled itself does for
orthogonal maps unless you change it.
