...
function GameOver({ label }: { label: string }) {
  // bind() runs once on mount and re-runs (with cleanup) after every
  // re-split — so a typewriter rebinds itself when `label` changes and
  // the old glyph objects are destroyed.
  ...
  return <SplitText ref={ref} charAnchor={0.5} style={{ fontSize: 48 }}>{label}</SplitText>;
}
```

Imperatively, `new UISplitText({ children, charAnchor, … })` exposes the
same `chars` / `words` / `lines` and an `onSplit(cb)` subscription.
