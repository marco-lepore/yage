...
YAGE's UI is Yoga-flexbox-based, but it tracks **web flexbox defaults** rather
than Yoga's raw ones — the divergence that otherwise causes text and children
to overflow instead of shrinking gracefully:

- **Flex children shrink by default (`flexShrink: 1`).** Yoga's raw default is
  `0`, which means a child reports its natural size and refuses to give space
  back — so a `Text` sharing a row with, say, an icon would spill past the
  container instead of wrapping. With the web default, children compress to fit
  and text wraps inside its slot. Set `flexShrink: 0` on a child that must keep
  its size (a fixed icon, a pinned sidebar); an explicit value always wins.

- **A dev-mode warning catches the leftovers.** When an in-flow child still
  overflows its container — a `flexShrink: 0` child too large to fit, or a
  wrapped label taller than a *fixed-height* button — YAGE logs a one-time
  `console.warn` pointing at the offending node. It's compiled out of
  production builds and stays quiet for intentional overflow (`overflow:
  "hidden"` containers, `position: "absolute"` children, and scroll views).

When a warning fires, fix it by giving the container more room, setting
`maxWidth`/`maxHeight`, letting the content wrap, or using
`truncate: "clip" | "ellipsis"` on the text (see [Text](#text)) or button.
