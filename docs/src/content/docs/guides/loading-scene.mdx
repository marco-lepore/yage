...
the preload, so `target.preload` is still inspected):
...
class GameScene extends Scene {
  ...
  constructor(readonly config: { level: number }) {
    super();
  }
}
...
The factory runs each time a load starts. A retry after a failed load calls
it again, so the retry builds a new target scene.
...
class Boot extends LoadingScene {
  readonly target = new GameScene();
  readonly minDuration = 0.5; // stays visible at least 0.5s
}
```

Wall-clock seconds. Ignores `timeScale`.
...
class Boot extends LoadingScene {
  readonly target = new GameScene();
  readonly transition = crossFade({ duration: 0.4 });
  override onEnter() {
    this.spawn(LoadingSceneProgressBar);
    this.startLoading();
  }
