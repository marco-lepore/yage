...
you'd rather defer until loading actually finishes:
...
Loader rejections rethrow by default, propagating to whoever called
`push`/`replace` on the loading scene. Override `onLoadError` to handle
them in-place:
...
class Boot extends LoadingScene {
  ...
  onLoadError(err: Error) {
    // Scene stays mounted — you can draw a retry UI or push an error scene
  }
}
```
