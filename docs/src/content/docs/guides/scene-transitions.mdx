...
irisReveal({
  duration: 800,
  center: { x: 0, y: 0 },           // bloom from the top-left
...
- `getSceneContainer(ctx, scene)` — returns the PIXI `Container` for a
  ...
  `filters` directly.
- `getVirtualBounds(ctx)` — returns `{ width, height }` of the coordinate
  space scene roots and `app.stage` operate in. Use it to size fullscreen
  overlays, masks, and slide distances.
...
:::caution[Don't read `app.screen` for sizing]
`app.stage` carries the responsive-fit transform, so its children — every
scene root, plus anything you `addChild` directly to the stage — operate
in **virtual-space pixels**. `app.screen` returns canvas/CSS pixels;
sizing a fullscreen overlay or mask from it silently shrinks or stretches
the geometry under any non-1.0 fit ratio (mobile letterbox is the common
case). Reach for `getVirtualBounds(ctx)` (or `renderer.virtualSize`)
instead.
:::
