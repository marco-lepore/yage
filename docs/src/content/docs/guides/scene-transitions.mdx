...
2. Both scenes coexist during the transition
3. `transition.end()` fires (can still reference both scenes)
4. The old scene is removed (`onExit`, entity destruction)
5. `scene:replaced` event emitted

### Queueing

Multiple `push`/`pop`/`replace` calls queue automatically — they don't cancel
each other. Each completes its transition before the next starts.

Calling `engine.scenes.clear()` cancels all in-flight and queued transitions
immediately.
