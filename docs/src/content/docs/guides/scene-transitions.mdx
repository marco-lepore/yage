...
```typescript
import { easeOutCubic } from "@yagejs/core";
...
```typescript
import { easeInQuad } from "@yagejs/core";
...
```typescript yage-context="engine,scene,component"
...
- `getSceneContainer(ctx, scene)` — returns the scene's root
  `DisplayContainer` (a PixiJS `Container`) so you can manipulate its
  `alpha`, `visible`, `position`, or `filters` directly.
...
`push`, `pop`, `replace`, and `popAll` return promises that resolve when the
operation, including any transition, has finished. `pop()` resolves to the
removed scene, or `undefined` when the stack was empty. Await a call before
code that needs the new stack, or mark a call you do not wait for with
`void`.
