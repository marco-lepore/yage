...
`engine.scenes` is the `SceneManager`. Inside a scene or a component, reach
it with `this.use(SceneManagerKey)` and pass the same options.
...
- `getSceneContainer(ctx, scene)` — returns the scene's root
  `DisplayContainer` so you can manipulate its `alpha`, `visible`,
...
`SceneManager.pop()` returns `Promise<Scene | undefined>`. It resolves with
the removed scene once the queued pop and its transition have run. `await` it,
or mark a call you don't wait on with `void`.
