...
**`chessboard({ duration?, rows?, cols?, color? })`** — Staggered
checkerboard wipe. Even-parity cells fade in over the first quarter,
odd-parity over the second; both parities are fully opaque at the
mid-point so the scene swap is hidden under the cover; the same stagger
runs in reverse over the second half to reveal the destination.
Defaults to 700ms, a 6×10 grid, and black.

```typescript
import { chessboard } from "@yagejs/renderer";

chessboard() // 700ms 6×10 black grid
chessboard({ rows: 4, cols: 6, color: 0x1a1a2e, duration: 900 })
```
