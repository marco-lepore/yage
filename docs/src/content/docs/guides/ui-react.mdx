...
records, maps, sets, lists, single-cell values, and `createStore` records.
One overload per shape, plus a selector escape hatch.

```tsx
import { createStore, useStore } from "@yagejs/ui-react";
...
Each `useStore(source)` returns the leaf's natural snapshot:
...
The compound returned by `defineStore` is intentionally not accepted — read
individual leaves instead, so subscription granularity stays per-leaf:
