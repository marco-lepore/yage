...
| `<Stack>` | Overlay panel — fills its parent, children stack via `position="absolute"` |
...
### `<Stack>` — opinionated overlay primitive

`<Stack>` is a `<Panel>` that defaults to `width: "100%"`, `height: "100%"`,
and `position: "relative"`. Drop absolute-positioned children inside to
stack them at (0, 0) — modal backdrops, HUD layers, and badge markers:
