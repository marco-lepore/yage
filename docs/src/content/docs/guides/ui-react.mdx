...
function Score({ score }: { score: number }) {
  const valid = <Text>{`Score: ${score}`}</Text>; // one string child
  // yage-expect-error TS2322
  const invalid = <Text>Score: {score}</Text>; // mixed children
  return valid;
}
```
...
function Selection({
  ...
  selected: boolean;
  highlightBg: NonNullable<ComponentProps<typeof Panel>["bg"]>;
}) {
  return (
    <>
...
Optional props accept explicit `undefined`. Required inputs, such as a Pixi
widget's view slots, must still be supplied.
...
function Example({ progress }: { progress: number }) {
  return (
...
```tsx
import { ZStack, Panel, Text, Button } from "@yagejs/ui-react";
...
```tsx
import { Tooltip, Button } from "@yagejs/ui-react";

function Example({ save }: { save: () => void }) {
  return (
    <Tooltip
...
```tsx
import { Tooltip, Panel, Text, Image } from "@yagejs/ui-react";
import type { TextureRef } from "@yagejs/renderer";

function Example({ swordIcon }: { swordIcon: TextureRef }) {
  return (
    <Tooltip
      placement="right"
      content={
...
function PauseButton() {
  ...
  return (
    <Button
      onClick={() => scenes.push(new PauseScene())}
      width={100}
      height={40}
    >
      Pause
...
import { useStore, Text } from "@yagejs/ui-react";
...
const value = createValue({ default: () => "ready" });
const compound = createStore((s) => ({ score: s.counter() }));
...
import { useQuery, Text } from "@yagejs/ui-react";
...
```tsx
import { useSceneSelector, Text } from "@yagejs/ui-react";
