...
The bubble is out of the layout flow (it never reflows siblings) and
unclipped (it can extend past the trigger). It is start-aligned on the
cross axis rather than centered — precise centering needs a measured size,
so compose a custom overlay with `<ZStack>` if you need it.
