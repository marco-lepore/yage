...
Props: `direction` (`"vertical"` default, or `"horizontal"`), `gap`,
`padding`, `scrollbar` (an auto-hiding thumb, on by default), `bg`, and
`onScroll(offset)`. Keep anything that should stay fixed — a header, a
footer button — as a **sibling** of `<ScrollView>`, never a child.
