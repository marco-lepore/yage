...
`"ui"` layer, positioned by the `anchor` and optional `offset`. Pass
`layer: "<name>"` to mount on any layer you've declared on
`Scene.layers` — a `space: "world"` layer is the canonical way to host
diegetic UI that follows the camera:

```typescript
const root = new UIRoot({
  layer: "overlay",              // must be declared on Scene.layers
