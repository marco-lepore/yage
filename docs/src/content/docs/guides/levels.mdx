...
```typescript yage-group="project" yage-file="Crate.ts"
...
section below](#pairs-and-places). `param.object`, `param.array` and
`param.json` carry a value with members, a list of them, and anything else, and
are [their own section below](#values-with-a-shape). `param.custom` carries a
value your game decodes and `param.color` a colour, and they are [their own
section below](#values-your-game-decodes). `param.entityRef` names another
...
```typescript title="src/Torch.ts" yage-group="project" yage-file="Torch.ts"
import { Entity, Transform } from "@yagejs/core";
import {
  defineLevelAsset,
  defineLevelEntity,
...
// Your game's own component.
declare class SlimeBrain extends Component {
...
```typescript title="src/Door.ts" yage-group="door" yage-file="Door.ts"
import { Component, Entity } from "@yagejs/core";
import {
  defineLevelEntity,
  defineParams,
...
```typescript title="src/Slime.ts" yage-group="points"
...
A wave has a list of spawns. A spawn has a type and a delay. `param.object`
...
// Your game's own components.
declare class Chest extends Component {
  constructor(item: string, count: number);
}
declare class Spawner extends Component {
...
class Wave extends Entity {
  ...
  setup(params: ParamsOf<typeof WaveParams>): void {
    ...
    for (const spawn of params.spawns) {
      spawner.queue(spawn.type, spawn.delay);
    }
  }
}
```

The members are declared the way the schema's own fields are, and every one is
...
Nothing checks what is inside it and the editor offers only the text of it, so
use it when the shape is open — a table another tool exports, a blob a system
of your own parses. Declare the shape for everything else, and you get controls
and findings with it.

### Values your game decodes

Sometimes the value your game needs is not the value a file can hold. A facing
...
// Your game's own type and component.
declare class Direction {
  static fromName(name: string): Direction;
}
declare class Walk extends Component {
...
`editor` says which control the editor draws, borrowed from one of the plain
...
const LampTex = texture("assets/lamp.png");
...
class Lamp extends Entity {
  setup(params: ParamsOf<typeof LampParams>): void {
    this.add(new SpriteComponent({ texture: LampTex, tint: params.tint })); // 0xffcc88
  }
}
```

The level file holds `"#rgb"` or `"#rrggbb"`, so it reads as the colours it
...
```typescript title="src/Switch.ts" yage-group="door" yage-file="Switch.ts"
...
// Your game's own placeable type and component.
...
```typescript title="src/Waypoint.ts"
...
The annotation is on the schema alone: `setup()` keeps its ordinary
...
```typescript title="src/levelProject.ts" yage-group="project" yage-file="levelProject.ts"
...
```typescript yage-group="project" yage-file="ForestScene.ts" yage-check="syntax" yage-reason="Imports a level file as a Vite JSON module, and the snippet checker resolves no JSON modules."
...
3. **`levelAssets`** hands those assets to `preload`, so they are loaded before
   `onEnter` runs.
4. **`instantiateLevel`** creates the entities. Keep what it returns when your
   scene needs to reach them later — see [the returned
   `LevelInstance`](#placing-the-same-level-twice).
...
```typescript yage-group="twice" yage-context="scene"
import { instantiateLevel, type PreparedLevel } from "@yagejs/level";

declare const room: PreparedLevel; // prepared once, like `forest` above

instantiateLevel(scene, room, { namespace: "room-west" });
instantiateLevel(scene, room, {
  namespace: "room-east",
...
```typescript yage-group="twice" yage-context="scene"
const level = instantiateLevel(scene, room, { namespace: "room-west" });
...
```typescript
import { readLevel } from "@yagejs/level";
...
Each result carries the field's name, its kind, whatever that kind needs, and
its default:
...
- **The placement's transform arrives after `setup()` returns.** Anything that
  reads `Transform.position` in `setup()`, or in a component's `onAdd()`, sees
  where the entity was before the level placed it — and a component that
  captures a position there and writes it back every frame moves the placement
  to the wrong place with no error anywhere. Read the placed pose from a
  component's `onEnable()` or from an update.
