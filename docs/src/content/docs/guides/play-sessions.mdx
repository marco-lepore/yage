...
scenario spawned. The same loop runs in an `inspector.drive` on the game page,
where there is no `scene` — read state through `inspector.getEntityPosition`,
`inspector.getComponentData`, or an extension the game registers with
`inspector.addExtension`.
