...
// Contracts of the game scene, ground sensor, and level-specific queries.
// Their implementations belong beside the level this scenario exercises.
...
```typescript yage-context="browser"
async function measureRun(atExit: () => boolean) {
  const i = window.__yage__.inspector;
  let lastX = 0;
...
async function captureOpenDoor(inspector: Inspector, doorOpen: () => boolean) {
