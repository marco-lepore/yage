...
so a layout change that moves the button moves the click with it. The node's
`layout` field is measured from its parent node and gives no position on the
canvas. The nodes are the tree under `ui.root` in a scene snapshot, which
`inspector.snapshot().scenes` lists and `inspector.snapshotScene(name)`
returns. In a scene with several surfaces, `ui.root` is a wrapper node whose
children are the surfaces' root nodes, and the wrapper itself cannot be
clicked. A lab scenario reads the snapshot from `window.__yage__.inspector`
too, because its drive context has no snapshot method.
