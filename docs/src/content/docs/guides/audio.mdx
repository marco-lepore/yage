...
For save and load, snapshots store only the alias string — re-register the
buffer under the same alias during boot, before restoring, the same way you
registered it on first run.
