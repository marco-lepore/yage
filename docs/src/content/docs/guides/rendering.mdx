...
For linear and radial gradients, use `linearGradient` / `radialGradient` instead
of importing `FillGradient` from `pixi.js`. Stops use yage's numeric color +
alpha pairs (no CSS color strings), and the result plugs straight into `.fill()`:
...
By default, coordinates and radii are in **local** space (the 0-1 range of the
filled shape), so one gradient instance scales to any rect you apply it to. Pass
`space: "global"` to treat them as absolute pixels instead.

Gradients own a GPU texture — call `.destroy()` in a component's `onRemove()`
when you're done with them.

## Text

For world-space and HUD text, use `TextComponent`. It's Transform-synced like a
sprite, serializable, and takes a style object that mirrors CSS font properties.
...
Update the text at runtime with `setText()` / `setStyle()`, or tweak `tint` and
`alpha` directly. For flexbox-laid-out UI rather than free-positioned text, use
`UIText` from `@yagejs/ui` instead.
