...
```typescript
// Sample the hold BEFORE mirroring the button: fireActionUp (via
// setActionHeld(..., false)) resets getHoldDuration to 0 on the release frame.
const charge = input.getHoldDuration("attack");
input.setActionHeld("attack", chargeButton.isHeld);

if (input.isJustReleased("attack")) {
  const power = Math.min(charge / 1000, 1); // 0..1 over 1s
