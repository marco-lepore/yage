...
The engine holds the press rather than guessing how long is too long. Discard
it on resume by consuming it and ignoring the result:
...
class GameScene extends Scene {
  onResume() {
    // Drop a jump buffered before the pause instead of firing it now.
...
Presses made *during* the pause are already handled: an action whose group is
disabled records no press at all, so disabling the gameplay group while the menu
is up keeps menu input out of the buffer.
:::
