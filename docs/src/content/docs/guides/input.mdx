...
**seconds** (matching engine time everywhere else):
...
configuration. `isJustHeldFor` fires once, on the frame the hold crosses the
threshold; the release helpers fire once, on the frame the action fully
releases (for an action bound to several keys, letting go of one key while
another still holds it doesn't count):
...
// The exact hold length at release (0 except on the release frame).
const held = input.getReleaseDuration("attack"); // seconds
```

`getReleaseDuration` is captured at the release edge, so it stays valid on the
frame the action is released — no need to sample `getHoldDuration` before the
release lands.
