...
overlayEl.addEventListener("pointerdown", (e) => {
  // Forward to the canvas so UI listeners underneath still see the tap.
...
```typescript
// Each frame, mirror the button's held state onto the action.
input.setActionHeld("attack", chargeButton.isHeld);

if (input.isJustReleased("attack")) {
  const power = Math.min(input.getHoldDuration("attack") / 1000, 1); // 0..1 over 1s
