...
Each graphical component therefore publishes a derived **render facet** on the
snapshot, computed on demand from its live display object:
...
// (first graphical component the entity added) and per component:
label?.render;           // { bounds: { x, y, width, height } | null, visible }
label?.components.find((c) => c.type === "SplitTextComponent")?.render;
...
// `render` is typed as the base facet, so widen it to read the SplitText extras
// (SplitTextRenderFacet is exported from @yagejs/renderer):
...
`serialize()`, so it does not affect save/load. `SpriteComponent`,
`AnimatedSpriteComponent`, `GraphicsComponent`, `TextComponent`, and
`SplitTextComponent` all implement it; a custom component opts in by exposing
an `inspectRender(): RenderFacetSnapshot` method, which the Inspector
duck-types.
