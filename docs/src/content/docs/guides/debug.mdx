...
engine. Unlike the debug overlays above, it lives in **`@yage/core`**, not
`@yage/debug` — you don't need the `DebugPlugin` installed to use it. Every
...
```typescript
import { Logger, LogLevel, LoggerKey } from "@yage/core";
...
```typescript
import { Engine, LogLevel } from "@yage/core";
