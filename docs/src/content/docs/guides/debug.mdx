...
// World-space bounds + resolved visibility, surfaced at the entity level
// (first graphical component) and per component:
...
camera and responsive `fit` transform — and `null` when nothing is painted
(e.g. an empty `Graphics`). `visible` is the component's own (local) flag; Pixi
