...
- `createList<T>` / `s.list`: `add(item): number` (id), `remove(id): boolean`, `get(id)`, `update(id, partial)`, `list()`, `size()`, `clear()`. With a `keyBy` option, also `findId(key)`, `getByKey(key)`, and `upsert(key, item): number` (merge if the key exists, else insert). Without `keyBy`, the keyed methods throw.
...
on `hydrate`. Keys are last-writer-wins: if two items share a key, the index
points at the most recently written one.
