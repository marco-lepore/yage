...
export const day       = createValue<number>({ defaults: () => 1 });
...
it("round-trips a record", async () => {
  const settings = createRecord({ defaults: () => ({ music: 0.8, sfx: 1.0 }) });
  settings.set({ music: 0.3 });
  await save.persist("settings", settings);

  const next = createRecord({ defaults: () => ({ music: 0.8, sfx: 1.0 }) });
