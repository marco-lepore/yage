...
// Tween a single property (durations in seconds)
pc.run(Tween.to(sprite, "alpha", 0, 0.3, easeOutQuad));
...
pc.run(
  new Sequence()
    .call(() => sprite.alpha = 1)
    .wait(0.2)
    .then(Tween.to(sprite, "alpha", 0, 0.4, easeOutQuad))
    .call(() => entity.destroy())
    .start(),
);
```
...
pc.run(
  new Sequence()
    .parallel(
      Tween.to(sprite, "alpha", 0, 0.3, easeOutQuad),
...
pc.run(
  new Sequence()
    .then(Tween.to(sprite, "alpha", 0.5, 0.5, easeInOutQuad))
    .then(Tween.to(sprite, "alpha", 1.0, 0.5, easeInOutQuad))
    .loop()
    .start(),
);
...
```typescript
pc.run(Tween.to(sprite, "alpha", 0, 0.3, easeOutQuad), { tags: ["vfx"] });
pc.run(Tween.to(sprite, "scale", 2, 0.3, easeOutQuad), { tags: ["vfx"] });
...
function telegraph(enemy: Entity, attack: () => void) {
  ...
  pc.run(
    new Sequence()
      ...
      .then(Tween.to(sprite, "alpha", 0.5, 0.2, easeInOutQuad))
      .wait(0.3)
      .call(() => {
        sprite.alpha = 1;
