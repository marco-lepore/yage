...
A track needs at least two keyframes, sorted by time, and a `duration` and
`speed` that are finite and greater than zero — anything else throws when the
animation starts. The first keyframe does not have to sit at time 0; a track
