...
Both clocks share the same pause and time-scale behavior: scene pause stops
them, and global, scene, and per-entity time scales apply to both. Fixed-clock
processes advance after the physics step and before component
`fixedUpdate(dt)` calls. The slot's clock is set at creation;
...
any one entity; hanging it on an entity's `ProcessComponent` ends the timer
when that entity is destroyed.

```typescript
import { Process, makeSceneScopedQueue } from "@yagejs/core";

const waves = makeSceneScopedQueue(processSystem, scene, { clock: "fixed" });
waves.run(Process.delay(30, () => spawnWave(2)));
```
...
`processSystem.cancelForScene(scene, "waves")` cancels by tag across both
clocks.
