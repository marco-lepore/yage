...
`start()`/`restart()` overrides cannot change it. Engine-level pools
(`ProcessSystem.add` / `addForScene`) always run on frame time.
...
A scoped process queue carries the clock for everything it enqueues. The
entity queue routes through the entity's `ProcessComponent` and cancels only
what it put there:
