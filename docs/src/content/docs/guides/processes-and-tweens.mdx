...
Why it matters: on the frame clock, the hitbox at `time: 0.18` is first seen by
a different fixed step on every run, because frame time and fixed time drift
apart. When a slow frame hits the `maxFixedStepsPerFrame` cap, the simulation
advances less than the real time that passed, while a frame-clock timeline
keeps running at full speed. On `"fixed"` the beats stay in step with the world
they drive.
