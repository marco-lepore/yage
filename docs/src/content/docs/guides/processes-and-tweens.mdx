...
`maxFixedStepsPerFrame` cap, the simulation advances fewer steps than the
rendered-frame time that passed, while a frame-clock timeline keeps running at
full speed. On `"fixed"` the beats stay in step with the world they drive.
