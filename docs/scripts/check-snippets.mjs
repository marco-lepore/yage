...
// A context list names hosts and a wrapper, so its order carries no meaning.
// parseMetadata rejects duplicates, so equal length plus inclusion is equality.
function sameContexts(a, b) {
  return a.length === b.length && a.every((context) => b.includes(context));
}
  ...
  snippets.forEach((snippet) => {
    ...
    if (
      ...
      (parts[0].metadata.check !== snippet.metadata.check ||
        !sameContexts(parts[0].metadata.contexts, snippet.metadata.contexts))
    ) {
      snippet.diagnostics.push({
        code: "directive",
        line: snippet.line,
