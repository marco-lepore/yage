...
directory is local working state and is excluded from Git, so the queue may
exist only in the repository's main worktree.
...
4. If no queue exists, stop and report that the local planning state is
   unavailable. Do not reconstruct the queue from conversation memory.
...
self-contained. The detailed product and system contracts are in the five
editor documents linked from the queue.

Before implementing anything, read "What this is optimizing for" in
`plans/yage-tools/editor-implementation-plan.md`. The rules in this workflow
exist for one purpose: an architecture that stays coherent across many
iterations by agents who share no memory, neither fragmenting into disjointed
code nor growing more structure than the editor needs. A rule applied against
that purpose is applied wrong.
...
| `run the next level editor item`           | Decide, implement, run the item's required independent review passes, and verify the first actionable item using `decision-rule.md`. Stop at any failed gate. |
...
2. Reverify dated claims against the current worktree. Plans describe intent.
   The current code determines what is feasible.
...
Each pass uses a distinct independent reviewer. No reviewer may be the item's
...
not a review of a diff. The adversarial pass runs on an item that touches
persistence, concurrency, or a public API. That means anything that writes a
file, orders operations across tabs or requests, or adds a public interface (an API, a file format, a config schema) that a
game will depend on. An item that touches none of those (a panel layout, a pure view
helper) does not need it. Say in the item's record which passes ran and why.
...
attacker is not a finding here. The "Security boundaries" section of
`editor-system-design.md` states the same limit.
...
3. **Big-picture coherence review.** Check the diff in the full engine and
   ...
   editor documents. For each changed module, read its section in
   `editor-module-architecture.md`, which states that module's public surface,
   ownership, and allowed imports. Then inspect completed and future queue
   items. Look for
...
- An item's first review round runs every pass the item requires. Prompt each
...
- After corrections, one focused reviewer verifies the corrected code and the
  ...
  the design.
- A re-review prompt describes the current state of the code first. A
  returning reviewer re-checks only its own prior findings, not the other
  passes' dispositions.
...
The three passes above examine a diff for engineering defects. None of them
checks whether a developer can build a level with the result. A slice can pass
all three and still leave the tool unusable.

A slice gate therefore also runs a use pass, which is not a review:
...
2. Record in the gate item every point where the tool was slow, confusing, or
   unable to do what the task needed. Write each point down as it happens, not
...
Reading the code, running the Playwright end-to-end path, and reviewing the
diff do not substitute for the use pass. The friction record, not the test
count, is the evidence that the slice met its purpose.
