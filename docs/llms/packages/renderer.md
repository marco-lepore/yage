...
- Safe-area padding on the `container`: `RendererPlugin` fits the game inside the container's content box, so the padding keeps it clear of the notch, corners and home indicator with no renderer config. The `body` background shows in the padding; match it to `backgroundColor`.
- Fullscreen button: place it inside the `container` (the element that goes fullscreen) so it stays visible. Show it only when `document.fullscreenEnabled === true || document.webkitFullscreenEnabled === true` (the prefixed flag covers iPad Safari < 16.4). It is not `true` on iPhone (no element fullscreen) or in an iframe without `allow="fullscreen"`. On iPad, Safari overlays its own exit button and a swipe down exits fullscreen. Blur the button after a click so game keys (Enter, Space) do not press it again.
