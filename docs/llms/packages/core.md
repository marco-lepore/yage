...
`Logger` writes to the console by default in dev builds (gated by `isDev()`, tree-shakable in production the same way as `devWarn`). Pass `logger: { output }` in the `Engine` config to replace it; `LogLevel.None` silences everything. The `output` sink itself is guarded: a sink that throws, or returns a promise that rejects, is disabled from its first observed failure on, with one console message, instead of taking down whatever was being reported. Calls an async sink already received before its first rejection settles still run.
